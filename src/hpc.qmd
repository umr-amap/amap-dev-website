...
HPC clusters usually provide a lot of scientific software and libraries that you can use. They are installed in a module system that you can load and unload. Some usefull commands are:
...
### Submit jobs
All HPC clusters use a job scheduler to manage the ressources and the jobs submitted by users. The most common job scheduler is `SLURM` (Simple Linux Utility for Resource Management). This is the scheduler use at `meso@lr`, but also àdastra` and `jean-zay`.
...
We refer to `SLURM` documentation to see all the options available, but we list here the most basic and usefull ones to write a batch script.
...
|--ntasks-per-core |  | Number of tasks per core (ussually 1) |
...
Different types of ressources are available on the cluster, as listed in @sec-mesolr:
...
> warning
> A adapter et tester: from Colin Prieur !!
>
1.Réserver un noeud GPU intéractif
2.Checker la version du driver nvidia avec "nvidia-smi" sur le noeud
3.Quitter le noeud GPU
4.Load le module de versioning approprié sur un noeud d'interface de base (pour moi c'est le python anaconda)
5.Installer un environnement "mock" avec juste le pip et bourer la version pytorch/R (je sais pas ce que vous utilisez) compatible avec le driver NVIDIA avec pip (voir ce lien https://pytorch.org/)
6.Rajouter les librairies spécialisées qui sont utiles pour vous dans la version.

6.Lancer un noeud GPU avec l'environnement approprié et voilou !
