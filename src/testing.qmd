...
Julia has a standardised way of setting up tests using the `Test.jl` package. There are extensions (and alternatives for VSCode such as `testitem` practices).

The general workflow is the following:
- Add a `test` folder to your directory root
- Activate that folder to create a test-specific environment and add the `Test` package to it
- Write your test functions within that folder
- Add a `runtests.jl` file to the `test` folder
...
- Add one or multiple `@testset`, which group tests together for legibility and compartimentalisation, and write your true/false tests within each `@testset` with the `@test` macro.
- You can then run these tests manually by running the `runtests.jl` script, calling `Pkg.test()`, or add it to your CI/CD with a CI.yml file that makes use of the `julia-actions/julia-runtest` Github Action.
