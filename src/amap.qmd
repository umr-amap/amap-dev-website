...
Recently, `Anaconda` has [updated it's terms of services for academia and research](https://www.anaconda.com/blog/update-on-anacondas-terms-of-service-for-academia-and-research). As being part of research institutes of more that 200 persons, we are now subject to a licensing fee to use `Anaconda` products. However, we can still use the `conda` tool.

While the `conda` tools is free and open-source, `Anaconda` (a graphical interface that uses conda) is a licensed product.

Also, `conda` uses "channels" to gets access to softwares and libraries. Some are licensed and managed by `Anaconda` while other are free and open.
...
- using the `Anaconda` distribution, as it is a licensed product. If you have it installed, you should uninstall it and consider other options bellow. Make sure you keep your existing environments.
...
- use the [`miniforge` distribution](https://conda-forge.org/download/). It is free to use and is configure to use `conda-forge` as the default channel (which is free too). This is the recommended distribution to use at AMAP.
...
`CIRAD` has decided to ban the access to theses licensed channels on their network, which means your `conda` will through an error if you try to access them when you're at `CIRAD` or using their VPN.
